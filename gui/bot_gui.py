...
from PySide6.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QPushButton, QTextEdit, QHBoxLayout, QLabel)
from PySide6.QtCore import QTimer

# Import du système de logs avancé
try:
    from config import advanced_log_manager
    from tools.advanced_logging import LogEntry, LogLevel
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        if ADVANCED_LOGGING_AVAILABLE:
            ...
            # Bouton pour l'interface améliorée
            enhanced_btn = QPushButton("🚀 Interface Améliorée")
            enhanced_btn.clicked.connect(self.open_enhanced_gui)
            buttons_layout.addWidget(enhanced_btn)
            
            # Bouton pour le visualiseur de logs
            logs_btn = QPushButton("📋 Visualiseur de Logs")
            ...
            # Label d'information
            info_label = QLabel("✅ Système de logs avancé disponible")
            info_label.setStyleSheet("color: green; font-weight: bold;")
            layout.addWidget(info_label)
        else:
            info_label = QLabel("⚠️ Système de logs avancé non disponible")
    ...
    def open_enhanced_gui(self):
        """Ouvre l'interface graphique améliorée"""
        try:
            import subprocess
            import sys
    ...
    def open_log_viewer(self):
        ...
        try:
            ...
            # Lance le visualiseur de logs dans un nouveau processus
            subprocess.Popen([
                sys.executable, 
                "tools/log_viewer_gui.py"  # Nouveau chemin vers l'outil déplacé
            ], cwd=os.path.dirname(os.path.abspath(__file__)))
            
            self.append_log("[INFO] Visualiseur de logs lancé")
            
        except Exception as e:
            self.append_log(f"[ERROR] Erreur lors du lancement du visualiseur: {e}")

def main():
    """Fonction main pour lancement depuis start_neuro.py"""
    app = QApplication(sys.argv)
    window = MainWindow()
    window.show()
    return app.exec() == 0  # Retourne True si succès
